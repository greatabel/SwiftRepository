...
public class DetailViewModel {
    ...
    func addPayback() {
        let names = nameComponents
        let amount = (self.amount as NSString).doubleValue
        let payback = Payback(firstName: names[0], lastName: names[1],
                              createdAt: NSDate(), amount: amount)
    ...
    func savePayback() {
        let names = nameComponents
        let amount = (self.amount as NSString).doubleValue
        context.editPayback(index, firstName: names[0], lastName: names[1],
                            amount: amount, updated: NSDate())
    }
