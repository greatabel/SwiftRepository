...
var  data = JSON.parse(json_dataB);
	...
	if(mypublisher =="" || mypublisher == null){
		
		$("#publisher_OuterDiv").hide();
	}
	else
	{
		$("#publisher_OuterDiv").show();
...
var image_lilist = '';
 ...
 //    </div>';
  ...
  '<span class="glyphicon glyphicon-chevron-right"></span></a>   </div>';
     ...
     // alert(mycontent);
     if(myimgList!=null && myimgList.length >0){
...
switch(myfrom){ 
case "recent":    
  $.getJSON('http://61.183.87.134:8090/mobile/proDetail.jspx?id='+myid, function(data) {
...
case "listpage": 
  ...
  $.getJSON('http://61.183.87.134:8090/mobile/bu_detail.jspx?type='+mytype+'&id='+myid, function(data) {
